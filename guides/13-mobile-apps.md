...
| **B — Tauri 2 mobile** | Rust core + system WebView now targets iOS/Android alongside desktop (v2 stable since Oct 2024, actively updated). The right call when a **Tauri desktop app already exists** and the UI is webview-friendly — one codebase, six platforms. | Best understood as "desktop-first with mobile reach": webview UI feel, large-payload IPC costs, and a younger mobile plugin ecosystem than RN. A phone-first consumer product will fight it. |
