...
3b. **Permanent directives**: ask about anything that must never be forgotten (language policy, forbidden deps, security invariants). Offer this one proactively to Korean users: `모든 대화·질문·보고는 한국어로 한다 (코드·식별자는 영어)`. Write each to `.claude/rules/project-directives.md` (always-loaded). Design preferences from step 2 land in PROJECT.md and, if strong, as a directive (e.g., `아이콘은 lucide-react만 사용`).
