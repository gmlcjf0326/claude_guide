...
- Pick ONE component system and route it through your own `src/ui/` wrapper layer; never mix two libraries. Strong React options: **Astryx** (Meta's open-source, agent-ready system — accessible components, theming via CSS custom properties; **beta and React-only**, so pin versions) or **shadcn/ui** (full source ownership). Non-React stacks: shadcn-vue / shadcn-svelte / Angular Material or the platform's own components. This is a real decision — run the guides/12-design-system.md §2 comparison before committing (boring beats novel for infrastructure).
...
- Enforcement is stage-gated: when `docs/PROJECT.md` declares a production/paid quality bar, emoji-as-icon, unloaded/default fonts on branded UI (unless explicitly chosen), and dead image hotlinks are review-BLOCKING on user-facing diffs. For prototype/internal/hackathon stages they are advisory — note them, don't fail the review.

## Style files, not inline styles (효율 원칙 — review-blocking)
- **No static `style=` attributes on individual tags.** They can't be cached or reused, carry maximal specificity, kill theming, and bloat HTML. Honest exceptions (the ONLY four): JS-computed dynamic values (`translateX(${x}px)`), CSS-variable injection only (`style={{'--i': i}}` with real styling in the sheet), truly per-instance runtime values, and compile-to-inline targets where inline styles ARE the required output format (HTML email — react-email/MJML — and similar).
...
## Korean public-sector (공공기관) work — skip this section entirely unless the client is a Korean public institution
